...
**Two timer patterns exist — use the right one:**
- **Gameplay simulation** (bomb fuse, explosions, movement): Use `deltaTicks` decrement. These timers should feel the same locally regardless of network timing.
- **Network coordination** (grace period, failsafe, timeout, mesh stagger): Use `TickCount()` wall-clock comparison. These timers coordinate between machines or wait on external events and must fire at consistent wall-clock time regardless of frame rate. Pattern: store `TickCount()` at start, check `TickCount() - startTick >= threshold` for expiry. The deltaTicks cap at 10 causes these timers to drift on slow machines (Mac SE at <6fps).

- `gGame.deltaTicks` — actual elapsed ticks since last frame (set in main loop, capped at 10)
- Decrement gameplay timers by `gGame.deltaTicks`, never by 1
- Constants: `BOMB_FUSE_TICKS=180` (~3s), `EXPLOSION_DURATION_TICKS=20` (~0.33s), `DEATH_FLASH_TICKS=60` (~1s), `DISCONNECT_GRACE_TICKS=90` (~1.5s), `MESH_STAGGER_PER_RANK=30` (~0.5s/rank), `GAME_OVER_FAILSAFE_TICKS=120` (~2s), `GAME_OVER_TIMEOUT_TICKS=180` (~3s), `HEAP_CHECK_INTERVAL_TICKS=1800` (~30s)
...
- C89/C90 (Retro68/RetroPPC cross-compiler) + PeerTalk SDK v1.11.2, clog v1.4.1, Classic Mac Toolbox (TickCount) (007-timer-fixes)
- N/A (no persistent storage; all state in memory) (007-timer-fixes)
- C89 / C90 only. Retro68 (m68k-apple-macos) and RetroPPC (powerpc-apple-macos) cross-compilers. `-std=c89 -Wall -Wextra`. No `//` comments, no mixed declarations, no VLAs, no `stdint.h`, no C99 features. clog uses variadic macros (C99) — do NOT add `-pedantic`. + Classic Mac Toolbox (QuickDraw, QDOffscreen, Memory Manager, Resource Manager); PeerTalk SDK v1.11.2; clog v1.4.1. (008-perf-hotpath)
...
- 005-network-authority: Owner-authoritative bomb explosions (only bomb owner broadcasts MSG_BOMB_EXPLODE and MSG_BLOCK_DESTROYED, ~67% TCP traffic reduction). Authority-based game over (lowest-rank connected player sends MSG_GAME_OVER, 120-tick failsafe for non-authority). Graceful game-over shutdown (90-tick grace period before TCP teardown, fixes Mac SE disconnect reason 2). Mesh stagger (rank * 30 ticks delay before first connect after MSG_GAME_START, fixes 6200 TCP connect failure). Heap monitoring (256KB threshold, checked at init + every 30s). Protocol v5 (behavioral change only, v4-compatible). New constants: DISCONNECT_GRACE_TICKS(90), MESH_STAGGER_PER_RANK(30), GAME_OVER_FAILSAFE_TICKS(120), GAME_OVER_TIMEOUT_TICKS(180), LOW_HEAP_WARNING_BYTES(262144), HEAP_CHECK_INTERVAL_TICKS(1800). New GameState fields: disconnectGraceStart, meshStaggerStart, gameOverAuthority, localGameOverDetected, gameOverFailsafeStart, gameOverTimeoutStart, heapCheckTimer. New net.c functions: Net_IsLowestRankConnected(), Net_GetLocalRank().
